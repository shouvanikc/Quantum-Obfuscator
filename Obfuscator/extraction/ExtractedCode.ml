...
module Nat =
 struct
  ...
  let modulo x0 y =
    (fun fO fS n -> if n=0 then fO () else fS (n-1))
      (fun _ -> y)
      (fun y' -> sub y' (snd (divmod x0 y' 0 y')))
      y
 end
...
let rec swap_qubits dim q1 q2 = function
| [] -> []
| g :: t1 ->
  (match g with
   | App1 (u, n) ->
...
let h_create_rule' _ q l =
  Some (app ((h q) :: []) (app ((h q) :: []) l))

(** val h_create_rule :
    int -> int -> int -> rzk_ucom_l -> rzk_Unitary gate_app list option **)

let rec h_create_rule dim n q l = match l with
| [] -> h_create_rule' dim q []
| h0 :: t1 ->
  ((fun fO fS n -> if n=0 then fO () else fS (n-1))
     (fun _ -> h_create_rule' dim q l)
     (fun n' ->
     let l' = h_create_rule dim n' q t1 in
     (match l' with
      | Some l1 -> Some (app (h0 :: []) l1)
      | None -> l'))
     n)

(** val add_obfuscated_H :
    int -> int -> int -> int -> rzk_ucom_l -> rzk_Unitary gate_list option **)

let add_obfuscated_H dim m n q l =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> None)
    (fun _ ->
    let l1 = h_create_rule dim n q l in
    (match l1 with
     | Some l2 ->
       let l3 = propagate l2 (rz_commute_rules q) [] m in
       (match l3 with
        | Some l4 ->
          let l5 = propagate l4 ((x_commute_rule q) :: []) [] m in
          (match l5 with
           | Some l6 -> Some l6
           | None -> l3)
        | None ->
          let l5 = propagate l2 ((x_commute_rule q) :: []) [] m in
          (match l5 with
           | Some l6 -> Some l6
           | None -> l1))
     | None -> None))
...
let x_create_rule' _ q l =
  Some (app ((x q) :: []) (app ((x q) :: []) l))

(** val x_create_rule :
    int -> int -> int -> rzk_ucom_l -> rzk_Unitary gate_app list option **)

let rec x_create_rule dim n q l = match l with
| [] -> x_create_rule' dim q []
| h0 :: t1 ->
  ((fun fO fS n -> if n=0 then fO () else fS (n-1))
     (fun _ -> x_create_rule' dim q l)
     (fun n' ->
     let l' = x_create_rule dim n' q t1 in
     (match l' with
      | Some l1 -> Some (h0 :: l1)
      | None -> l'))
     n)

(** val add_obfuscated_X :
    int -> int -> int -> int -> rzk_ucom_l -> rzk_Unitary gate_list option **)

let add_obfuscated_X dim m n q l =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> None)
    (fun _ ->
    let l1 = x_create_rule dim n q l in
    (match l1 with
     | Some l2 ->
       let l3 = propagate l2 (rz_commute_rules q) [] m in
       (match l3 with
...
let rec rz_create_rule dim n i q l = match l with
| [] -> rz_create_rule' dim i q []
| h0 :: t1 ->
  ((fun fO fS n -> if n=0 then fO () else fS (n-1))
     (fun _ -> rz_create_rule' dim i q l)
     (fun n' ->
     let l' = rz_create_rule dim n' i q t1 in
     (match l' with
      | Some l1 -> Some (h0 :: l1)
      | None -> l'))
     n)

(** val add_obfuscated_Rz :
    int -> int -> int -> int -> int -> rzk_ucom_l -> rzk_Unitary gate_list
    option **)

let add_obfuscated_Rz dim m n i q l =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> None)
    (fun _ ->
    let l1 = rz_create_rule dim n i q l in
    (match l1 with
     | Some l2 ->
       let l3 = propagate l2 (rz_commute_rules q) [] m in
       (match l3 with
...
(** val cNOT_create_rule' :
    int -> int -> int -> rzk_ucom_l -> rzk_Unitary gate_app list option **)

let cNOT_create_rule' _ q1 q2 l =
  Some (app ((cNOT q1 q2) :: []) (app ((cNOT q1 q2) :: []) l))

(** val cNOT_create_rule :
    int -> int -> int -> int -> rzk_ucom_l -> rzk_Unitary gate_app list option **)

let rec cNOT_create_rule dim n q1 q2 l = match l with
| [] -> cNOT_create_rule' dim q1 q2 []
| h0 :: t1 ->
  ((fun fO fS n -> if n=0 then fO () else fS (n-1))
     (fun _ -> cNOT_create_rule' dim q1 q2 l)
     (fun n' ->
     let l' = cNOT_create_rule dim n' q1 q2 t1 in
     (match l' with
      | Some l1 -> Some (h0 :: l1)
      | None -> l'))
     n)

(** val add_obfuscated_CNOT :
    int -> int -> int -> int -> int -> rzk_ucom_l -> rzk_Unitary gate_list
    option **)

let add_obfuscated_CNOT dim m n q1 q2 l =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> None)
    (fun _ ->
    let l1 = cNOT_create_rule dim n q1 q2 l in
    (match l1 with
     | Some l2 ->
       let l3 = propagate l2 (cNOT_commute_rules q1 q2) [] m in
       (match l3 with
        | Some l4 ->
          let l5 = propagate l4 ((x_commute_rule q1) :: []) [] m in
          (match l5 with
           | Some l6 ->
             let l7 = propagate l6 (rz_commute_rules q1) [] m in
             (match l7 with
              | Some l8 -> Some l8
              | None -> l5)
           | None ->
             let l7 = propagate l4 (rz_commute_rules q1) [] m in
             (match l7 with
              | Some l8 -> Some l8
              | None -> l3))
        | None ->
          let l5 = propagate l2 ((x_commute_rule q1) :: []) [] m in
          (match l5 with
           | Some l6 ->
             let l7 = propagate l6 (rz_commute_rules q1) [] m in
             (match l7 with
              ...
              | None -> l5)
           | None ->
             let l7 = propagate l2 (rz_commute_rules q1) [] m in
             (match l7 with
              | Some l8 -> Some l8
              | None -> l1)))
     | None -> Some ((pDAG 0) :: [])))
...
(** val expand_circuit :
    int -> int -> rzk_ucom_l -> rzk_Unitary gate_app list **)

let rec expand_circuit dim2 dim1 = function
| [] -> []
| g :: t1 ->
  (match g with
   | App3 (_, _, _, _) -> []
   | x0 -> x0 :: (expand_circuit dim2 dim1 t1))

(** val offset_circuit :
    int -> int -> rzk_ucom_l -> rzk_Unitary gate_app list **)

let rec offset_circuit dim2 dim1 = function
| [] -> []
| g :: t1 ->
  (match g with
   | App1 (u, m) ->
     (App1 (u, (sub (add m dim1) dim2))) :: (offset_circuit dim2 dim1 t1)
   | App2 (u, m, n) ->
     (App2 (u, (sub (add m dim1) dim2),
       (sub (add n dim1) dim2))) :: (offset_circuit dim2 dim1 t1)
   | App3 (_, _, _, _) -> [])

(** val merge_random :
    int -> rzk_ucom_l -> rzk_ucom_l -> int list -> rzk_ucom_l **)

let rec merge_random dim top bottom mergerand =
  match top with
  | [] -> bottom
  | h1 :: t1 ->
    (match bottom with
     | [] -> top
     | h2 :: t2 ->
       (match mergerand with
        | [] -> top
        | n :: t3 ->
          ((fun fO fS n -> if n=0 then fO () else fS (n-1))
             (fun _ -> h1 :: (merge_random dim t1 bottom t3))
             (fun n0 ->
             (fun fO fS n -> if n=0 then fO () else fS (n-1))
               (fun _ -> h1 :: (merge_random dim t1 bottom t3))
               (fun _ -> h2 :: (merge_random dim top t2 t3))
               n0)
             n)))
...
let rec make_list base size =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> [])
    (fun n ->
    app (base :: []) (make_list (add base (Pervasives.succ 0)) n))
    size

(** val add_teleport :
    int -> int -> int -> int -> rzk_ucom_l -> rzk_Unitary gate_app list **)

let add_teleport _ orig ans des l =
  app l
    ((h ans) :: ((cNOT ans des) :: ((cNOT orig ans) :: ((h orig) :: (
    (cNOT ans des) :: ((h des) :: ((cNOT orig des) :: ((h des) :: ((h orig) :: (
    (h ans) :: []))))))))))

(** val add_teleport_front :
    int -> int -> int -> int -> rzk_ucom_l -> rzk_Unitary gate_app list **)

let add_teleport_front dim orig ans des l =
  app
    ((h ans) :: ((cNOT ans des) :: ((cNOT orig ans) :: ((h orig) :: (
    (cNOT ans des) :: ((h des) :: ((cNOT orig des) :: ((h des) :: ((h orig) :: (
    (h ans) :: [])))))))))) (swap_qubits dim orig des l)

(** val teleport_connect :
    int -> int list -> int list -> int list -> int list -> rzk_ucom_l ->
    rzk_ucom_l **)

let rec teleport_connect dim orig ans des flags l =
  match orig with
  | [] -> l
  | h1 :: t1 ->
    (match ans with
       ...
       (match des with
        ...
        | h3 :: t3 ->
          (match flags with
           ...
           | h4 :: t4 ->
             if (=) h4 0
             then teleport_connect dim t1 t2 t3 t4
                    (add_teleport dim h1 h2 h3 l)
             else teleport_connect dim t1 t2 t3 t4
                    (add_teleport_front dim h1 h2 h3 l))))

(** val expand_teleport : int -> int list -> rzk_ucom_l -> rzk_ucom_l **)

let rec expand_teleport dim flags l =
  teleport_connect (add (add dim dim) dim) (make_list 0 dim)
    (make_list dim dim) (make_list (add dim dim) dim) flags
    (expand_circuit dim (add (add dim dim) dim) l)

(** val expand_teleport_short :
    int -> int list -> int list -> rzk_ucom_l -> rzk_ucom_l **)

let rec expand_teleport_short dim qubits flags l =
  let ls = length qubits in
  teleport_connect (add (add dim ls) ls) qubits (make_list dim ls)
    (make_list (add dim ls) ls) flags
    (expand_circuit dim (add (add dim ls) ls) l)

(** val add_random_obfuscated_CNOTs :
    int -> int list -> int list -> int list -> int list -> rzk_ucom_l ->
    rzk_ucom_l **)

let rec add_random_obfuscated_CNOTs dim nrand mrand l1rand l2rand l =
  match nrand with
  | [] -> l
  | h1 :: t1 ->
    (match mrand with
       ...
       (match l1rand with
        ...
        | h3 :: t3 ->
          (match l2rand with
           | [] -> l
           | h4 :: t4 ->
             (match add_obfuscated_CNOT dim h2 h1 h3 h4 l with
              | Some l' -> add_random_obfuscated_CNOTs dim t1 t2 t3 t4 l'
              | None -> add_random_obfuscated_CNOTs dim t1 t2 t3 t4 l))))

(** val get_random_circuit :
    int -> int list -> int list -> int list -> rzk_ucom_l **)

let rec get_random_circuit dim crand qrand arand =
  match crand with
  | [] -> []
  | h0 :: t1 ->
    (match qrand with
       ...
       (match arand with
        ...
        | h'' :: t'' ->
          ((fun fO fS n -> if n=0 then fO () else fS (n-1))
             ...
             (fun fO fS n -> if n=0 then fO () else fS (n-1))
               (fun _ ->
               (h h') :: (get_random_circuit dim t1 t' t''))
               (fun n0 ->
               (fun fO fS n -> if n=0 then fO () else fS (n-1))
                 (fun _ ->
                 (t0 h') :: (get_random_circuit dim t1 t' t''))
                 (fun n1 ->
                 (fun fO fS n -> if n=0 then fO () else fS (n-1))
                   (fun _ ->
                   (tDAG h') :: (get_random_circuit dim t1 t' t''))
                   (fun n2 ->
                   (fun fO fS n -> if n=0 then fO () else fS (n-1))
                     (fun _ ->
                     (p h') :: (get_random_circuit dim t1 t' t''))
                     (fun n3 ->
                     (fun fO fS n -> if n=0 then fO () else fS (n-1))
                       (fun _ ->
                       (pDAG h') :: (get_random_circuit dim t1 t' t''))
                       (fun n4 ->
                       (fun fO fS n -> if n=0 then fO () else fS (n-1))
                         (fun _ ->
                         ...
                         (fun fO fS n -> if n=0 then fO () else fS (n-1))
                           ...
                           (fun fO fS n -> if n=0 then fO () else fS (n-1))
                             ...
                             (fun fO fS n -> if n=0 then fO () else fS (n-1))
                               (fun _ ->
                               (cNOT h'
                                 (add h' (Nat.modulo (Pervasives.succ 0) dim))) :: 
...
(** val add_random_circuit :
    int -> int list -> int list -> int list -> rzk_ucom_l -> rzk_Unitary
    gate_app list **)

let add_random_circuit dim crand qrand arand l =
  app l (get_random_circuit' dim crand qrand arand)

(** val add_local_obfuscation :
    int -> int list -> int list -> int list -> int list -> int list ->
    rzk_ucom_l -> rzk_ucom_l **)

let rec add_local_obfuscation dim orand qorand norand morand aorand l =
  match orand with
  | [] -> l
  | h1 :: t1 ->
    (match qorand with
           ...
           | h4 :: t4 ->
             (match aorand with
              ...
              | h5 :: t5 ->
                ((fun fO fS n -> if n=0 then fO () else fS (n-1))
                   (fun _ ->
                   add_local_obfuscation dim t1 t2 t3 t4 t5 l)
                   (fun n ->
                   (fun fO fS n -> if n=0 then fO () else fS (n-1))
                     (fun _ ->
                     match add_obfuscated_H dim h4 h3 h2 l with
                     | Some l' -> add_local_obfuscation dim t1 t2 t3 t4 t5 l'
                     | None -> add_local_obfuscation dim t1 t2 t3 t4 t5 l)
                     (fun n0 ->
                     (fun fO fS n -> if n=0 then fO () else fS (n-1))
                       (fun _ ->
                       match add_obfuscated_X dim h4 h3 h2 l with
                       | Some l' ->
                         add_local_obfuscation dim t1 t2 t3 t4 t5 l'
                       | None -> add_local_obfuscation dim t1 t2 t3 t4 t5 l)
                       (fun n1 ->
                       (fun fO fS n -> if n=0 then fO () else fS (n-1))
                         (fun _ ->
                         match add_obfuscated_Rz dim h4 h3 h5 h2 l with
                         | Some l' ->
                           add_local_obfuscation dim t1 t2 t3 t4 t5 l'
                         | None -> add_local_obfuscation dim t1 t2 t3 t4 t5 l)
                         (fun _ ->
...
(** val obfuscate :
    int -> int -> rzk_ucom_l -> int list -> int list -> int list -> int list
...
(** val obfuscate_secondary :
    int -> rzk_ucom_l -> int list -> int list -> int list -> int list -> int
    list -> int list -> int list -> int list -> int list -> rzk_ucom_l **)

let obfuscate_secondary dim l orand qorand norand morand arand nrand mrand l1rand l2rand =
  let firstpass = add_local_obfuscation dim orand qorand norand morand arand l
  in
  add_random_obfuscated_CNOTs dim nrand mrand l1rand l2rand firstpass
