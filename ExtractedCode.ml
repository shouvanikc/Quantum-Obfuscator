 ...
 struct
  ...
  let rec add n m =
    (fun fO fS n -> if n=0 then fO () else fS (n-1))
      (fun _ -> m)
      (fun p0 -> Pervasives.succ (add p0 m))
      n
...
(** val add_comp_circuit_bottom :
    int -> int -> rzk_ucom_l -> rzk_ucom_l -> rzk_ucom_l **)

let rec add_comp_circuit_bottom dim1 dim2 l = function
| [] -> l
| g :: t1 ->
  (match g with
   | App1 (u, m) ->
     app (add_comp_circuit_bottom dim1 dim2 l t1) ((App1 (u,
       (sub (add m dim1) dim2))) :: [])
   | App2 (u, m, n) ->
     app (add_comp_circuit_bottom dim1 dim2 l t1) ((App2 (u,
       (sub (add m dim1) dim2), (sub (add n dim1) dim2))) :: [])
   | App3 (_, _, _, _) -> l)

(** val combine_circuits :
    int -> int -> rzk_ucom_l -> rzk_ucom_l -> rzk_ucom_l **)

let combine_circuits dim1 dim2 l1 l2 =
  add_comp_circuit_bottom (add dim1 dim2) dim2
    (expand_circuit dim1 (add dim1 dim2) l1) l2
       ...
       (match l1rand with
        ...
        | h3 :: t3 ->
          (match l2rand with
           | [] -> l
           | h4 :: t4 ->
             (match add_obfuscated_CNOT dim h1 h2 h3 h4 l with
              | Some l' -> add_random_obfuscated_CNOTs dim t1 t2 t3 t4 l'
              | None -> add_random_obfuscated_CNOTs dim t1 t2 t3 t4 l))))

(** val shift_list : int -> int list -> int list **)

let rec shift_list m = function
| [] -> []
| h0 :: t1 -> (Nat.add h0 m) :: t1

(** val add_obfuscated_correlations :
    int -> int -> int list -> int list -> int list -> int list -> rzk_ucom_l
    -> rzk_ucom_l -> rzk_ucom_l **)

let add_obfuscated_correlations dim1 dim2 nrand mrand l1rand l2rand l1 l2 =
  let l = combine_circuits dim1 dim2 l1 l2 in
  add_random_obfuscated_CNOTs (add dim1 dim2) nrand mrand l1rand l2rand l

(** val obfuscate :
    int -> int -> rzk_ucom_l -> int list -> int list -> int list -> int list
    -> int list -> int list -> int list -> rzk_ucom_l **)

let obfuscate dim dim2 l crand qrand arand nrand mrand l1rand l2rand =
  let ancilla = get_random_circuit dim2 crand qrand arand in
  add_obfuscated_correlations dim dim2 nrand mrand l1rand
    (shift_list dim l2rand) l ancilla
